    ...
    jQuery.ajax({
        ...
        success: function (msg) { 
            if (msg.ExamState != undefined) {
                ...
                {
                    ShowSystemDialog("对不起，该数据已归档，不能进行解除！");
...
function SetCustomerInfo(msg, ID_Customer) {
    ...
    if (dataList0 != undefined) {
        item = dataList0[0];
        jQuery("#lblReportReceiptor").text(item.ReportReceiptor);
        jQuery("#lblReportReceiptedDate").text(item.ReportReceiptedDate);
        jQuery("#lblCustomerName").text(item.CustomerName);
        jQuery("#lblCustomerNameX").text(item.CustomerName);
        ...
        jQuery("#lblID_CustomerX").text(item.ID_Customer);
...
function DoUnlockReportState() {
    ...
    var msgContent = "您确认要解除[" + ID_Customer + "]对应的报告领取标志吗？";
