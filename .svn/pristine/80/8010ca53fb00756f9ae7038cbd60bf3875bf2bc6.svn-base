...
jQuery(document).ready(function () {
    ...
    flag = jQuery("#flag").val(); //3:预约 6:登记 9：团体登记
    if (flag == undefined) flag = 0; //参数为空则表明不显示体检类型（个人预约、个人登记、团体登记都可以做改签）
    //设置默认选中
...
function DoSearchCustomerInfoAndCustFeeInfo() { 
    ...
    var ErrorMsg = "";
    var card = KeyValue;
    var TypeCode = card.substring(1, 2); //TypeCode:3 个人预约 ,6个人登记 ,9团体登记,0表示不验证
    //如果不为个人登记或者团体登记，则验证
    if (flag == 0) {
    }
    else {
    ...
    jQuery.ajax({
        ...
        success: function (msg) {
            if (msg.ExamState != undefined) {
                ...
                {
                    ShowSystemDialog("对不起，该数据已归档，不能进行改签！");
...
function DoUpdateCustomerSubScribDate() {
    //判断是否打印指引单，如果打印了指引单则不允许改签预定体检时间
    //    var data = jQuery("#txtID_Customer").data("data");
    //    if (data != null) { 
    //        if (data.Is_GuideSheetPrinted == "True") {
    //            ShowSystemDialog("客户指引单已打印，不允许修改预定日期！");
    //            return false;
    //        }
    //    }
    ...
    if (CustomerName != "") {
        if (SubScribDate != OldSubScribDate) {
            ...
            //                }
            //            });
            var messageContent = "您确认要将客户[" + CustomerName + "]体检日期[" + OldSubScribDate + "]修改为[" + SubScribDate + "]吗？";
                    ...
                    callback: function () {
                        if (flag != 6 && flag != 9) {
                            ...
                            if (SubScribDate != CurDate) {
                                return ShowSystemDialog("客户[" + CustomerName + "]的体检日期必须选择当天！");
                            }
                            else {
                                DoUpdateCustomerSubScribDate_Ajax(CustomerName, ID_Customer, SubScribDate);
                            }
