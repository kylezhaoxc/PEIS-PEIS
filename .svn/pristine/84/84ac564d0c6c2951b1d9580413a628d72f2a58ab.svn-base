            ...
            jQuery(dataList).each(function(i, item) {
                ...
                if (modelName.toLowerCase() == "GetCustomerCharges".toLowerCase()) {
                    if (item.FeeChargeStaute == "√") {
                        CustCssStyle += " HidChechBox";  //如果是收费模块 已收费的项目不应该出现复选框
                    }
                }
                //如果是退费模块 只有未检查且已收费未退费的项目允许出现复选框 调整如果是已收费但是未开始体检的需要显示checkbox
                else if (modelName.toLowerCase() == "ReturnCustomerCharges".toLocaleLowerCase()) {
                    if (item.FeeChargeStaute == "√") {
                        if (item.Is_Examined == "未检") {
                            CustCssStyle = "";
                        }
                    }
                    else {
...
function UnChargeAllCallback(UnChargeType) {
    ...
    if (Invoice.length > 0) {
        ...
        , CustomerName: CustomerName, TotalCustFee: parseFloat(TotalCustFee).toFixed(curFixed), UpperWrite: UpperWrite, SubScribDate: SubScribDate
        };
        //提交Ajax请求
        jQuery.ajax({
            type: "POST",
