...
jQuery(document).ready(function () {
    ...
    jQuery("#idSelectSet").change(function () {
        if (jQuery("#idSelectSet").val() == "" && (jQuery("#idSelectSet").data("ID_Customer") == undefined || jQuery("#idSelectSet").data("PEID") == "")) { return; } // 如果选择的体检类型为空值 ，则不进行下面的操作
        ...
        if (curValue != "-1" || jQuery("#idSelectSet").data("PEID") != undefined) {
            var PEPackageID = jQuery("#idSelectSet").val(); //获取套餐         
            if (jQuery("#idSelectSet").data("PEID") != undefined && jQuery("#idSelectSet").data("PEID") != "") {
                data = { PEID: jQuery("#idSelectSet").data("PEID"), action: 'GetBusFeeByCustomerID' };
            }
            else {
...
function ShowElementInfo() {
    ...
    if (type.toLowerCase() == "edit") {
        ...
        if (jQuery('#modelName').val().toLowerCase() == "regist") {
            //jQuery("#lblRegiste").text("预约时间");//xmhuang 2013-11-23 注释掉，统一显示为体检日期
...
function SaveData(objElement) {
    ...
    jQuery("#tblList tbody tr td [name='yj']").each(function () {
        if (jQuery(this).parent().parent().find("td input[name='ItemCheckbox']").length > 0) {
            id_custfee = jQuery(this).parent().parent().attr("id_custfee"); //收费项目ID xmhuang 2013-09-25 新增用于规避出现双项目
            id_customer = jQuery(this).parent().parent().attr("id_customer"); //收费项目所对应的体检号
            id = jQuery(this).parent().parent().attr("id_fee"); //收费项目ID
            id_discounter = jQuery(this).parent().parent().attr("id_discounter"); //折扣人ID
            discountername = jQuery(this).parent().parent().attr("discountername"); //折扣人名称
            exist = jQuery(this).parent().parent().attr('exist'); //是否存在
            ID_Section = jQuery(this).parent().parent().attr('id_section'); //科室ID
            SectionName = jQuery(this).parent().parent().attr('sectionname'); //科室名称
...
function AddNewRegistOrSign(IsCheck) {
    ...
    jQuery("#lblAge").text(""); //质空年龄显示 xmhuang 2013-12-04
}
/**************通过身份证号码获取用户基本信息**********************/
/// <summary>
/// 更新客户基本信息并返回
            ...
            success: function (msg) {
                ...
                if (IsLoadCustomer == 2) {
                    ...
                    if (msg.dataList != undefined) {
                        ...
                        if (IDCard == item.IDCard && CustomerName == item.CustomerName)  //如果检索出来的客户证件号和姓名都相同则直接使用，否则使用从证件上读取的信息为准 xmhuang 2013-12-05
                        {
...
function SetCustomerInfo(dataList, selectedID_ArcCustomer, IsLoadCustomerInfo) {
    ...
    for (var c = 0; c < dataList.length; c++) {
        ...
        if (jQuery("#HeadImg").attr("IsExist") == 1)//xmhuang 2013-11-25 用于区分是从卡片读取或者是拍照时，不再设置头像,此设置已经在读卡和拍照时完成
        {
        ...
        jQuery("#HeadImg").removeAttr("IsExist"); // xmhuang 2013-11-25 使用后移除存在标记
            ...
            success: function (msg) {
                if (msg.dataList0.length > 0) {
                    ...
                    jQuery(msg.dataList0).each(function (i, item) {
                        jQuery("#lblHidOperateDate").text(item.OperateDate); //xmhuang 2014-01-03 绑定指引单打印时间
                        //绑定操作密级 xmhuang 2014-01-28
...
function RequestCustomerInfo(objID, Key, IsLoadCustomerInfo) {
    /********判断是否是从自定义身份证读取,如果是则需要后天通过身份证和姓名同时检索 xmhuang 2013-11-23 Begin**********/
    var IsGenerateCustomerCard = 0;
    var data = jQuery("#txtSearchX").data("data");
    if (data != null) {
        IsGenerateCustomerCard = data.IsGenerateCustomerCard;
    }
    /********判断是否是从自定义身份证读取,如果是则需要后天通过身份证和姓名同时检索 xmhuang 2013-11-23 End**********/
    ...
    jQuery.ajax({
        ...
        success: function (msg) {
            ...
            jQuery("#lblErrorMessage").text("");
            jQuery("#txtSearchX").removeData("data"); //xmhuang 2013-11-23 移除自定义身份证生成标记
            return false;
        }
    });
...
function DoTakePhoto() {
    ...
    //document.getElementById("HeadImg").src = "data:image/gif;base64," + CurBase64Photo + "";
    jQuery("#HeadImg").attr("src", "data:image/gif;base64," + CurBase64Photo + "");
    jQuery("#HeadImg").attr("Base64Photo", CurBase64Photo);
    jQuery("#HeadImg").attr("IsExist", 1); //xmhuang 2013-11-26 标记该头像已经存在，不用再重新设置
    //保存到数据库 xmhuang 2013-11-22 注释拍照不直接提交头像信息，在完成时统一保存
    //判断是否存在身份在信息
    //    var objID = "", Key = "";
    ...
    //    }
    //    if (objID != "" && Key != "") {
    //        PostImg(Base64Code, objID, Key, 0, 0);
    //    }
...
function ResetAllCustomerInfo() {
    ...
    jQuery("#txtBirthDay").val(""); //重置出生日期 xmhuang 2013-11-26
    jQuery("#HeadImg").attr("src", defalutImagSrc); //重置头像 xmhuang 2013-11-26
    jQuery("#HeadImg").attr("Base64Photo", ""); //重置头像Base64Photo属性 xmhuang 2013-11-26
    jQuery("#lblAge").text(""); //质空年龄显示 xmhuang 2013-12-04
...
function SearchCustomerInfo(objID, Key) {
    ...
    var KeyValue = jQuery.trim(jQuery("#" + objID).val());
    //    if (KeyValue == "") {
    //        jQuery("#lblErrorMessage").text("请您输入正确的证件号或体检号！");
    //        return false;
    //    }
...
function OpenUserNoIDCard() {
    ...
    var tipscontent = '<table class="ModifyPassword">' +
            '<tbody>' +
            '    <tr><td class="left">客户姓名：</td><td><input maxlength="10" name="txtCustomerNameX" id="txtCustomerNameX" style="ime-mode:atuo;"/> &nbsp;</td></tr>' +
            '    <tr><td class="left">出生日期：</td><td><input maxlength="10" name="txtCustomerBirthdayX" id="txtCustomerBirthdayX" onfocus="WdatePicker({dateFmt:\'yyyy-MM-dd\'})"/> &nbsp;</td></tr>' +
...
function FindReader(objID, Key, IsUpdateCustomerPhoto) {
    ...
    if (ret == 0) {
        ...
        jQuery("#slNation [value='" + CVR_IDCard.NationCode + "']").attr("selected", true);
        ...
        ShowQuickSelectNation(CVR_IDCard.NationCode, NationArray[CVR_IDCard.NationCode - 1]);          // 设置民族的已选项
...
function CaltAgeByBirthDay() {
    jQuery("#lblAge").text("(" + GetAgeByBirthday(jQuery("#txtBirthDay").val()) + ")岁");
    //    var year = jQuery("#txtBirthDay").val().substring(0, 4);
    //    var age = CurDate.substring(0, 4) - year;
...
function GetAgeByBirthday() {
    ...
    var age = -1;
    var today = CurDate; //  new Date();
    var todayArr = CurDate.split("-");
    var todayYear = todayArr[0]; //  today.substring(0, 4); // today.getFullYear();
    var todayMonth = todayArr[1]; // today.substring(6, 2);
    var todayDay = todayArr[2]; //today.substring(9, 2);
    var birthDay = birthDay;
    var birthDayArr = birthDay.split("-");
    var birthdayYear = birthDayArr[0]; // birthDay.substring(0, 4);
    var birthdayMonth = birthDayArr[1]; // birthDay.substring(6, 2);
    var birthdayDay = birthDayArr[2]; // birthDay.substring(9, 2);
...
var Gender = 0; //性别
var GenderName = "男"; //性别
var Married = 0, MarriageName = ""; //婚姻状态
function FYHTesterX() {
    setInterval("FYHTester()", 200);
}
function FYHTesterX_New() {
    setInterval("FYHTester_New()", 200);
}

function GetCurTime() {
    var d = new Date();
    var s = "";
    return (d.getMonth() + 1) + "/" + d.getDate() + "/" + d.getYear() + " " + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds();
}
...
/// 修改人：黄兴茂
/// 修改日期：2013-11-18
function Calc(curBeginTime, curEndTime) {
    try {
        var a1 = curBeginTime;
        var a2 = curEndTime;
        var b1 = parseInt(a1.split(":")[0], 10) * 3600 + parseInt(a1.split(":")[1], 10) * 60 + parseInt(a1.split(":")[2]);
        var b2 = parseInt(a2.split(":")[0], 10) * 3600 + parseInt(a2.split(":")[1], 10) * 60 + parseInt(a2.split(":")[2]);
        var timeSpan = "";
        timeSpan = Math.abs(parseInt(b1 - b2)); //Math.abs(parseInt((b1 - b2) / 60, 10)) + ":" + Math.abs((b1 - b2) % 60);
    }
    catch (e) {
        timeSpan = e;
    }
...
/// 修改日期：2013-11-08
function FYHTester() {
    if (ExcuteCount > MaxCount) {
        jQuery("#btnFYHTester").val("生成测试数据");
        return false;
    }
    var curBeginTime = GetCurTime();
    var curEndTime = "";
    curYear++; curMonth++; curDay++;
    if (curYear > 2013) curYear = 1880;
    if (curMonth > 12) curMonth = 1;
    if (curDay > 28) curDay = 1;
    var CustomerName = "FYHTester";
    var Birthday = "", TempBirthday = "", TempMonth = "", TempDay = "";

    var CurGender, CurGenderName, CurMarried, CurMarriageName;
    TempMonth = curMonth;
    TempDay = curDay;
    //    for (var curYear = 1880; curYear <= 2013; curYear++) {
    //        for (var curMonth = 1; curMonth <= 12; curMonth++) {
    //            for (var curDay = 1; curDay <= 28; curDay++) {
    if (curMonth.toString().length < 2) {
        TempMonth = "0" + curMonth;
    }
    if (curDay.toString().length < 2) {
        TempDay = "0" + curDay;
    }

    Birthday = curYear + "-" + TempMonth + "-" + TempDay; //生成出生日期

    //生成性别
    if (Gender == 0 || Gender == 2)
    { CurGender = 1; CurGenderName = "男"; Gender = 1; }
    else if (Gender == 1)
    { CurGender = 2; CurGenderName = "女"; Gender = 2; }

    //生成婚姻
    if (Married == 0 || Married == 3)
    { CurMarried = 1; CurMarriageName = "未婚"; Married = 1; }
    else if (Married == 1)
    { CurMarried = 2; CurMarriageName = "已婚"; Married++; }
    else if (Married == 2)
    { CurMarried = 3; CurMarriageName = "婚姻不详"; Married = 0; }
    ...
    var IDCard = "111111" + "" + Birthday.replace(/-/gi, "") + "" + twoRandStr + "" + Sex + "" + oneRandStr;
    var ID_Customer = ""; //体检号
    var ExamCard = IDCard; //一卡通
    CustomerName = CustomerName + Math.random().toString().substring(2, 10);
    var MobileNo = "15810601040";                 //电话
    var GuideNurse = -1;                          //导检护士ID
    var GuideNurseName = "";                      //导检护士姓名
    var Nation = curDay;                          //民族ID
    var NationName = NationArray[curDay];         //民族
    var ExamType = 1;                          //健康体检类型
    var ExamTypeName = "健康体检";             //健康体检类型名称
    var BusSet = 121;                             //套餐类型ID
    var BusPEPackageName = "健康体检菜单3（男）";            //套餐类型名称
    var ReportWay = 6; //报告领取方式
    var ReportWayName = "自取"; //报告领取方式
    var FeeWay = 1; //付费方式
    var FeeWayName = "现金"; //付费方式
    var OperateLevel = 10; //操作密级
    var OperateLevelName = ""; //操作密级
    var Email = "512938078@qq.com"; //Email
    var ExamPlace = 1; //体检地址
    var ExamPlaceName = "四川省人民医院新大楼2楼"; //体检地址
    var Note = "FYHTester......"; //备注
    var Country = 1;   //国家ID
    var CountryName = "中国"; //国家名称
    var Cultrul = -1; //教育背景
    var CultrulName = -1; //教育背景
    var SubScribDate = CurDate; //登记时间
    var Base64Photo = jQuery("#HeadImg").attr("Base64Photo");
    var modelName = "Sign"; //设置为登记
    var BusFeeItems = ''; //记录所有的套餐
    var AllIDFee = ""; //所有收费项目ID xmhuang 2013-10-30
    var ID_ArcCustomer = "";
    jQuery("#tblList tbody tr td [name='yj']").each(function () {
        //if (jQuery(this).parent().parent().find("td input[name='ItemCheckbox']").length > 0) {
        var id_custfee = ""; //收费项目ID xmhuang 2013-09-25 新增用于规避出现双项目
        var id_customer = ""; //收费项目所对应的体检号
    ...
    var qustData = { action: 'SaveData',
        ...
        ExamCard: ExamCard,
        Gender: CurGender,
        GenderName: encodeURIComponent(CurGenderName),
        BirthDay: encodeURIComponent(Birthday),
        Married: CurMarried,
        MarriageName: encodeURIComponent(CurMarriageName),
        MobileNo: MobileNo,
        RegisteDate: encodeURIComponent(RegisteDate),
        Register: Register,
        BusSet: BusSet,
        ...
        success: function (jsonMsg) {
            ...
            jQuery(jsonMsg.dataList).each(function (i, msg) {
                if (msg.success == "1") {
                    ...
                    jQuery("#btnFYHTester").val("写入[" + ExcuteCount + "次]耗时[" + Calc(curBeginTime, curEndTime) + "]秒");
                    ...
                    var ShowMsg = "";
                    if (Is_Subscribed == 1) {
                        jQuery("#lblErrorMessage").text("客户[" + jQuery("#txtCustomer").val() + "]成功完成预约！");
                    }
                    else {
                        jQuery("#lblErrorMessage").text("客户[" + jQuery("#txtCustomer").val() + "]成功完成登记！");
                    }

                }
                else {
                    jQuery("#lblErrorMessage").text(msg.Message);
                }
            });
        },
        complete: function () {
...
function FYHTester_New() {
    ...
    var qustData = { action: 'SaveData_New',
        modelName: modelName,
        type: "add",
        ID_ArcCustomer: ID_ArcCustomer,
        CardNum: IDCard,
        ...
        success: function (jsonMsg) {
            ...
            jQuery(jsonMsg.dataList).each(function (i, msg) {
                if (msg.success == "1") {
                    ExcuteCount++;
                    curEndTime = GetCurTime();
                    jQuery("#btnFYHTester_New").val("写入[" + ExcuteCount + "次]耗时[" + Calc(curBeginTime, curEndTime) + "]秒");
