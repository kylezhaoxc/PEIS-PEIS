...
function QueryPagesData(pageIndex) {
    if (pagerData != null) {
        ...
        var newcontent = '';
        var templateContent = jQuery("#" + curTempLateName + " tbody").html();
        ...
        for (var c = 0; c < curPageSize; c++) {
            ...
            if (item != undefined) {
                if (templateContent != null) {
                    if (curTempLateName == "GuideSheetReturnedSearchWorkLoadTemplate") {
                    ...
                    else if (curTempLateName == "GuideSheetReturnedWorkLoadTemplate") {
                        newcontent += templateContent.replace(/@Index/gi, rowNum + c + 1)
                     .replace(/@GuideSheetReturnedby/gi, item.GuideSheetReturnedby)
...
function GetGuideSheetReturnedWorkLoad() {
    ...
    jQuery.ajax({
        ...
        success: function (msg) {
            if (msg != undefined) {
                curTempLateName = "GuideSheetReturnedWorkLoadTemplate";
...
function GetGuideSheetReturnedSearchWorkLoad() {
    ...
    jQuery.ajax({
        ...
        success: function (msg) {
            if (msg != undefined) {
                curTempLateName = "GuideSheetReturnedSearchWorkLoadTemplate";
                pagerData = msg.dataList0;
                tempOperPageCount = 0;
                QueryPagesData(0);
                jQuery(msg.dataList1).each(function (i, item) {
