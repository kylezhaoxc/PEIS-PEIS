    ...
    jQuery("input[name='chkSectionQueryList']:radio:checked").each(function () {
        if (jQuery("#chkUserSection_" + jQuery(this).val()).val() == undefined) {
            selectedItemContent = UserSelectedSectionItemDataTempleteContent
                            .replace(/@SectionID/gi, jQuery(this).val())
...
/// 点击确定按钮（确定选择报告合并收费项目）
/// </summary>
function SelectFeeDataList2() {
    var selectedItemContent = "";
    var UserSelectedFeeItemDataTempleteContent = jQuery('#UserSelectedFeeItemDataTemplete').html();             //用户已经选择的报告合并收费项目列表模版
    if (UserSelectedFeeItemDataTempleteContent == undefined) { return; }

    jQuery("input[name='chkFeeQueryList']:radio:checked").each(function () {
        //if (jQuery("#chkFee_" + jQuery(this).val()).val() == undefined) {
        selectedItemContent = UserSelectedFeeItemDataTempleteContent
                            .replace(/@ID_Fee/gi, jQuery(this).val())
                            .replace(/@FeeName/gi, jQuery(this).attr("Feename"))
                            ;
        jQuery("#spanSelectFee").html(selectedItemContent);
        jQuery("#spanSelectFee").show();
        jQuery("#idSelectFee").val(jQuery(this).val());
        jQuery("#nameSelectFee").val(jQuery(this).attr("Feename"));
        jQuery("#spanFee").hide();
        //}
    });
...
/// 选中对应行
/// </summary>
function SetSelectedExamItem(ID_ExamItem) {
...
function GetSymptomDetailListByExamID(ID_ExamItem) {
    ...
    if (jQuery("#box_ExamItemSymptom_" + ID_ExamItem).is(":hidden")) {
        //jQuery("#box_ExamItemSymptom_" + ID_ExamItem).show();
    } else {
        jQuery("#box_ExamItemSymptom_" + ID_ExamItem).hide();
        return;
    }
    jQuery.ajax({
        type: "POST",
        ...
        success: function (jsonmsg) {
            if (jsonmsg != null && jsonmsg != "" && jsonmsg.dataList0.length > 0) {
                ...
                var SymptomListItemTempleteContent = jQuery('#NoDataSymptomListItemTemplete').html();
                jQuery("#box_ExamItemSymptom_" + ID_ExamItem + " .SymptomItemList").html(SymptomListItemTempleteContent);  // 显示每列的数据
                jQuery(".SymptomItemList").parent().parent().parent().hide();
                jQuery("#box_ExamItemSymptom_" + ID_ExamItem).show();
            }
        }
    });
...
//    }
    ...
    jQuery(symptomlist.dataList0).each(function (j, symptomitem) {
        SymptomItemCount++;
        examitemid = symptomitem.ID_ExamItem; // 检查项目ID
...
//    jQuery("#SymptomListItem").html(tempListContent);  // 显示每列的数据
    jQuery("#box_ExamItemSymptom_" + examitemid + " .SymptomItemList").html(tempListContent);  // 显示每列的数据

    SetTableEvenOddRowStyleQT(); // 奇偶行背景

    jQuery(".SymptomItemList").parent().parent().parent().hide();
    jQuery("#box_ExamItemSymptom_" + examitemid).show();
...
function GetSaveUserParams() {
    ...
    var VocationType = jQuery("#VocationType").val();        // 分类
    ...
    var UserInfoParams = {
        ID_User: ID_User,
        UserName: UserName, ID_Section: ID_Section,
        SectionName: SectionName,
...
///  判断用户是否具有相应的权限
/// </summary>
function GetUserHaveRight() {

    var isHaveAllUserPassword_Right = Is_LoginUserRight('A.D.02_AllUserPassword');    // 密码管理
    var isHaveUserRight_Right = Is_LoginUserRight('A.E.01_UserRight');    // 权限分配
    var isHaveClearLoginErrorCount_Right = Is_LoginUserRight('A.G.05_ClearLoginErrorCount');    // 错误次数清理
    
    if (isHaveAllUserPassword_Right == true) { jQuery("#spanAllUserPassword_Right").show(); } else { jQuery("#spanAllUserPassword_Right").remove(); }
    if (isHaveUserRight_Right == true) { jQuery("#spanUserRight_Right").show(); } else { jQuery("#spanUserRight_Right").remove(); }
    if (isHaveClearLoginErrorCount_Right == true) { jQuery("#spanClearLoginErrorCount_Right").show(); } else { jQuery("#spanClearLoginErrorCount_Right").remove(); }
...
function ShowFeeListBySet(feelist) {
    ...
    jQuery(feelist.dataList0).each(function (j, feeitem) {
        ...
        tempListContent += FeeListItemTempleteContent
                ...
                .replace(/@ID_Fee/gi, feeitem.ID_Fee)
                .replace(/@Price/gi, parseFloat(feeitem.Price).toFixed(2))
                .replace(/@Forsex00/gi, Forsex00)
                .replace(/@Forsex01/gi, Forsex01)
...
function OpenSetFeeRelOperWindow() {
    ...
    var PEPackageID = jQuery("input[name='SetRadio']:checked").val();
    if (PEPackageID == undefined) {
        ShowSystemDialog("请选择需要设置收费项目的套餐！");
        return;
    }
    var setname = jQuery("#rdi_Set_" + PEPackageID).attr("setname");
    setname = encodeURIComponent(setname);

    var tmpUrl = "/System/Config/Set/SetDetail.aspx?PEPackageID=" + PEPackageID + "&setname=" + setname + "&num=" + Math.random();
    art.dialog.open(tmpUrl,
    {
        width: 600,
        height: 390,
...
function GetSaveBusSetParams() {
    ...
    var Forsex = jQuery("input[name='radioForsex']:checked").val();               // 适用于
    var isBanned = jQuery("input[name='radioisBanned']:checked").val();   // 是否被禁用
    var BanDescribe = jQuery("#txtBanDescribe").val();              // 禁用说明
    BanDescribe = encodeURIComponent(BanDescribe);                  // 禁用说明 编码处理
...
function HideDialogStyle() {

//    jQuery(".aui_header").hide();
...
//    jQuery(".aui_s").hide();
//    jQuery(".aui_se").hide();
//    jQuery(".aui_content").css("padding:0px; margin:0px;");
//    jQuery(".aui_outer").addClass('touming');
...
/// 点击选中对应体检地址的单选按钮
/// </summary>
function SetExamPlaceChecked(ExamPlaceID) {
    jQuery("#rdiExamPlace_" + ExamPlaceID).attr("checked", true);
}

/// <summary>
/// 弹出体检地址设置页面
/// </summary>
function OpenEditExamPlaceWindow() {
    
    var ExamPlaceID = jQuery("input[name='ExamPlaceRadio']:checked").val();
    if (ExamPlaceID == undefined) {
        ShowSystemDialog("请选择你要修改的体检地址！");
        return;
    }
    OpenExamPlaceOperWindowParams(ExamPlaceID);
}

/// <summary>
/// 弹出新增体检地址页面
/// </summary>
function OpenExamPlaceOperWindow() {
    OpenExamPlaceOperWindowParams("");
}

/// <summary>
/// 弹出体检地址设置页面(体检地址ID编号)
/// </summary>
function OpenExamPlaceOperWindowParams(ExamPlaceID) {
    
    var url = '/System/Config/ExamPlace/ExamPlaceOper.aspx?num=' + Math.random();
    if (ExamPlaceID != "") {
        url = url + "&ExamPlaceID=" + ExamPlaceID;
    }
    art.dialog.open(url,
    {
        width: 400,
...
/// 保存体检地址参数。 (保存)
/// </summary>
function SaveExamPlaceInfo() {

    // 获取保存时提交的参数。 (参数提取)
    var ExamPlaceParams = GetSaveExamPlaceParams();
    ...
    jQuery.ajax({
        type: "POST",
        url: "/Ajax/AjaxConfig.aspx",
        data: ExamPlaceParams,         // 体检地址参数
        cache: false,
        dataType: "json",
        success: function (jsonmsg) {
            if (parseInt(jsonmsg) > 0) {

                if (jQuery("#ExamPlaceID").val() != "") {
                    parent.GetSingleExamPlaceInfo(jQuery("#ExamPlaceID").val(), "edit");
                }
                else {
                    parent.GetSingleExamPlaceInfo(jsonmsg, "add");
                }


                parent.ShowSystemDialogCloseDialog("保存体检地址成功!", jQuery("#IS_AutoCloseDialog").val());
                ...
                parent.ShowSystemDialogCloseDialog("体检地址名称：【" + jQuery("#txtExamPlaceName").val() + "】已经存在，请核对后再操作！");
            }
            else { parent.ShowSystemDialogCloseDialog("保存体检地址失败，请与技术人员联系!") }
        }
    });
...
/// 获取保存时提交的参数。 (参数提取)
/// </summary>
function GetSaveExamPlaceParams() {

    var ExamPlaceID = jQuery("#ExamPlaceID").val();                     // 体检地址ID
    var ExamPlaceName = jQuery("#txtExamPlaceName").val();                // 体检地址名称
    ExamPlaceName = encodeURIComponent(ExamPlaceName);                    // 体检地址名称 编码处理
    ...
    if (ExamPlaceName == "") {
        jQuery('#txtExamPlaceName').focus();
        parent.ShowSystemDialogCloseDialog("请输入体检地址名称!");
        return;
    }
    ...
    var ExamPlaceSaveParams = {
        ExamPlaceName: ExamPlaceName,
        ExamPlaceID: ExamPlaceID,
        Is_Default: Is_Default,
        DispOrder: DispOrder,
        action: 'SaveExamPlaceInfo',
        currenttime: encodeURIComponent(new Date())
    };

    return ExamPlaceSaveParams; // 返回拼接后的参数
...
/// 初始化体检地址设置页面
/// </summary>
function InitExamPlaceEditData() {
    if (jQuery("#ExamPlaceID").val() == "") {
        return;
    }
    // 重置保存按钮的大小
...
/// 弹出检查项目设置页面(检查项目ID编号)
/// </summary>
function OpenUserRightSet() {


    var UserID = jQuery("input[name='UserRadio']:checked").val();
    if (UserID == undefined) {
        parent.ShowSystemDialog("请选择需要分配权限的用户！");
        return;
    }

    var UserName = jQuery("#rdiUser_" + UserID).attr("username");
    var url = '/System/Config/User/UserRightSet.aspx?num=' + Math.random();
    if (UserID != "") {
        url = url + "&UserID=" + UserID + "&UserName=" + encodeURIComponent(UserName);
    }
    art.dialog.open(url,
    {
        width: 652,
...
/// 点击选中对应收费项目报告的单选按钮
/// </summary>
function SetFeeReportChecked(ID_FeeReport) {
    jQuery("#rdiFeeReport_" + ID_FeeReport).attr("checked", true);

    jQuery("#SelectedFeeReportLable").show();
    jQuery("#InputFeeReportCodeText").hide();
}

/// <summary>
/// 弹出收费项目报告设置页面
/// </summary>
function OpenEditFeeReportWindow() {

    var FeeReportID = jQuery("input[name='FeeReportRadio']:checked").val();
    if (FeeReportID == undefined) {
        ShowSystemDialog("请选择你要修改的收费项目报告！");
        return;
    }
    OpenFeeReportOperWindowParams(FeeReportID);
}

/// <summary>
/// 弹出新增收费项目报告页面
/// </summary>
function OpenFeeReportOperWindow() {
    OpenFeeReportOperWindowParams("");
}

/// <summary>
/// 弹出收费项目报告设置页面(收费项目报告ID编号)
/// </summary>
function OpenFeeReportOperWindowParams(FeeReportID) {

    var url = '/System/Config/Conclusion/FeeReportOper.aspx?num=' + Math.random();
    if (FeeReportID != "") {
        url = url + "&FeeReportID=" + FeeReportID;
    }
    art.dialog.open(url,
    {
        width: 450,
...
/// 保存收费项目报告参数。 (保存)
/// </summary>
function SaveFeeReportInfo() {

    // 获取保存时提交的参数。 (参数提取)
    var FeeReportParams = GetSaveFeeReportParams();
    ...
    jQuery.ajax({
        type: "POST",
        url: "/Ajax/AjaxConfig.aspx",
        data: FeeReportParams,         // 收费项目报告参数
        cache: false,
        dataType: "json",
        success: function (jsonmsg) {
            if (parseInt(jsonmsg) > 0) {

                if (jQuery("#ID_FeeReport").val() != "") {
                    parent.GetSingleFeeReportInfo(jQuery("#ID_FeeReport").val(), "edit");
                }
                else {
                    parent.GetSingleFeeReportInfo(jsonmsg, "add");
                }


                parent.ShowSystemDialogCloseDialog("保存收费项目报告成功!", jQuery("#IS_AutoCloseDialog").val());
                ...
                parent.ShowSystemDialogCloseDialog("收费项目报告名称：【" + jQuery("#nameSelectFee").val() + "】已经存在，请核对后再操作！");
            }
            else { parent.ShowSystemDialogCloseDialog("保存收费项目报告失败，请与技术人员联系!") }
        }
    });
...
/// 获取保存时提交的参数。 (参数提取)
/// </summary>
function GetSaveFeeReportParams() {

    var ID_FeeReport = jQuery("#ID_FeeReport").val();                       // 收费项目报告ID
    var ID_Fee = jQuery("#idSelectFee").val();                              // 收费项目ID
    var FeeName = jQuery("#nameSelectFee").val();                           // 收费项目报告名称
    FeeName = encodeURIComponent(FeeName);                                  // 收费项目报告名称 编码处理

    var ReportKey = jQuery("#txtReportKey").val();                             // 模板名称
    ReportKey = encodeURIComponent(ReportKey);                              // 模板名称 编码处理

    var Note = jQuery("#txtNote").val();                                       // 文本备注
    Note = encodeURIComponent(Note);                                        // 文本备注 编码处理
    ...
    var ImageUrl = jQuery("#showImageUrl").data("imagedata");       // 备注图片64位数据

    if (FeeName == "") {
        jQuery('#txtFeeInputCode').focus();
        parent.ShowSystemDialogCloseDialog("请输入收费项目报告名称!");
        return;
    }
    jQuery("#IsCanSaveInfo").val("True");    // 数据验证成功，标记可以进行保存

    var FeeReportSaveParams = {
        ID_FeeReport: ID_FeeReport,
        ID_Fee: ID_Fee,
        FeeName: FeeName,
        ReportKey: ReportKey,
        Is_Banned: Is_Banned,
        ...
        Note: Note,
        BanDescribe: BanDescribe,
        action: 'SaveFeeReportInfo',
        currenttime: encodeURIComponent(new Date())
    };

    return FeeReportSaveParams; // 返回拼接后的参数
...
/// 初始化收费项目报告设置页面
/// </summary>
function InitFeeReportEditData() {
    if (jQuery("#ID_FeeReport").val() == "") {
        return;
    }
    ...
    var selectedItemContent = "";
    if (jQuery("#idSelectFee").val() != "") {

        var UserSelectedFeeItemDataTempleteContent = jQuery('#UserSelectedFeeItemDataTemplete').html();             //已经选择的列表模版
        if (UserSelectedFeeItemDataTempleteContent == undefined) { return; }

        selectedItemContent = UserSelectedFeeItemDataTempleteContent
                            .replace(/@ID_Fee/gi, jQuery("#idSelectFee").val())
                            .replace(/@FeeName/gi, jQuery("#nameSelectFee").val())
                            ;
        jQuery("#spanSelectFee").html(selectedItemContent);
        jQuery("#spanSelectFee").show();
        jQuery("#spanFee").hide();
...
/// 点击选中对应检查项目分组的单选按钮
/// </summary>
function SetExamItemGroupChecked(ID_ExamItemGroup) {
    jQuery("#rdiExamItemGroup_" + ID_ExamItemGroup).attr("checked", true);

    jQuery("#SelectedExamItemGroupLable").show();
    jQuery("#InputExamItemGroupCodeText").hide();
}

/// <summary>
/// 弹出检查项目分组设置页面
/// </summary>
function OpenEditExamItemGroupWindow() {

    var ExamItemGroupID = jQuery("input[name='ExamItemGroupRadio']:checked").val();
    if (ExamItemGroupID == undefined) {
        ShowSystemDialog("请选择你要修改的检查项目分组！");
        return;
    }
    OpenExamItemGroupOperWindowParams(ExamItemGroupID);
}

/// <summary>
/// 弹出新增检查项目分组页面
/// </summary>
function OpenExamItemGroupOperWindow() {
    OpenExamItemGroupOperWindowParams("");
}

/// <summary>
/// 弹出检查项目分组设置页面(检查项目分组ID编号)
/// </summary>
function OpenExamItemGroupOperWindowParams(ExamItemGroupID) {

    var url = '/System/Config/Exam/ExamItemGroupOper.aspx?num=' + Math.random();
    if (ExamItemGroupID != "") {
        url = url + "&ExamItemGroupID=" + ExamItemGroupID;
    }
    art.dialog.open(url,
    {
        width: 450,
...
/// 保存检查项目分组参数。 (保存)
/// </summary>
function SaveExamItemGroupInfo() {

    // 获取保存时提交的参数。 (参数提取)
    var ExamItemGroupParams = GetSaveExamItemGroupParams();
    ...
    jQuery.ajax({
        type: "POST",
        url: "/Ajax/AjaxConfig.aspx",
        data: ExamItemGroupParams,         // 检查项目分组参数
        cache: false,
        dataType: "json",
        success: function (jsonmsg) {
            if (parseInt(jsonmsg) > 0) {

                if (jQuery("#ID_ExamItemGroup").val() != "") {
                    parent.GetSingleExamItemGroupInfo(jQuery("#ID_ExamItemGroup").val(), "edit");
                }
                else {
                    parent.GetSingleExamItemGroupInfo(jsonmsg, "add");
                }


                parent.ShowSystemDialogCloseDialog("保存检查项目分组成功!", jQuery("#IS_AutoCloseDialog").val());
                ...
                parent.ShowSystemDialogCloseDialog("检查项目分组名称：【" + jQuery("#txtExamItemGroupName").val() + "】已经存在，请核对后再操作！");
            }
            else { parent.ShowSystemDialogCloseDialog("保存检查项目分组失败，请与技术人员联系!") }
        }
    });
...
/// 获取保存时提交的参数。 (参数提取)
/// </summary>
function GetSaveExamItemGroupParams() {

    var ID_ExamItemGroup = jQuery("#ID_ExamItemGroup").val();               // 检查项目分组ID
    var ExamItemGroupName = jQuery("#txtExamItemGroupName").val();          // 分组名称
    ExamItemGroupName = encodeURIComponent(ExamItemGroupName);              // 分组名称 编码处理

    var DispOrder = jQuery("#txtDispOrder").val();                          // 模板名称
    DispOrder = encodeURIComponent(DispOrder);                              // 模板名称 编码处理
    ...
    if (ExamItemGroupName == "") {
        jQuery('#txtExamItemGroupName').focus();
        parent.ShowSystemDialogCloseDialog("请输入检查项目分组名称!");
        return;
    }
    jQuery("#IsCanSaveInfo").val("True");    // 数据验证成功，标记可以进行保存

    var ExamItemGroupSaveParams = {
        ID_ExamItemGroup: ID_ExamItemGroup,
        ExamItemGroupName: ExamItemGroupName,
        DispOrder: DispOrder,
        Is_Banned: Is_Banned,
        Note: Note,
        BanDescribe: BanDescribe,
        action: 'SaveExamItemGroupInfo',
        currenttime: encodeURIComponent(new Date())
    };

    return ExamItemGroupSaveParams; // 返回拼接后的参数
...
/// 初始化检查项目分组设置页面
/// </summary>
function InitExamItemGroupEditData() {
    if (jQuery("#ID_ExamItemGroup").val() == "") {
        return;
    }
...
/// 弹出收费项目明细设置页面
/// </summary>
function OpenExamItemGroupRelOperWindow() {
    ...
    var tmpUrl = "/System/Config/Exam/ExamItemGroupRelOper.aspx?ExamItemGroupID=" + ExamItemGroupID + "&num=" + Math.random();
    art.dialog.open(tmpUrl,
    {
        width: 600,
        height: 390,
