...
function changepass() {
    ...
    $.ajax({
        ...
        type: "post",
        data: {OldPassword: mpass, NewPassword: newpass, action: 'ModifyUserPasswordInfo',currenttime: encodeURIComponent(new Date())},
        timeout: 2000,
        success: function (jsonmsg) {
            if (parseInt(jsonmsg) > 0){
                ...
                $("#changepass").after(" &nbsp;&nbsp;<span class='tipscolor tipsfont' id='tip'>密码修改失败，请与管理员联系</span>");
                $("#mpass").val("").focus();
                $("#newpass").val("");
                $("#renewpass").val("");
                setTimeout("removedomid(tip)", 3000);
        ...
        error: function () {
            $("#changepass").after(" &nbsp;&nbsp;<span class='tipscolor tipsfont' id='tip'>修改密码请求失败请重试，或与管理员联系</span>");
            $("#mpass").val("").focus();
            $("#newpass").val("");
            $("#renewpass").val("");
            setTimeout("removedomid(tip)", 3000);
